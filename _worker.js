...
function generateRemark(index, port, address, cleanIPs, protocol, configType) {
    ...
    return `X-DNL-X ${index} - ${protocol}${type} - ${addressType} : ${port}`;
}
...
async function renderHomePage (env, hostName, fragConfigs) {
	...
	<head>
        ...
        <title>X-DNL-X ${panelVersion}</title>
	...
	<body>
		<h1>X-DNL-X <span style="font-size: smaller;">${panelVersion}</span> 🆒</h1>
    ...
    warpEndpoints.split(',').forEach( (endpoint, index) => {
        ...
        if (client === 'singbox' || client === 'hiddify') {
            let singboxOutbound = buildSingboxWarpOutbound(
                client === 'hiddify' ? `X-DNL-X Warp Pro ${index + 1} 🦁☀️` : `X-DNL-X Warp ${index + 1} 🦁☀️`, 
                warpIPv6, 
                privateKey, 
                publicKey, 
                endpoint,
        ...
        if (client === 'clash') {
            let clashOutbound = buildClashWarpOutbound(`X-DNL-X Warp ${index + 1} 🦁☀️`, warpIPv6, privateKey, publicKey, endpoint, reserved, '');
        ...
        for (let i = 0; i < 2; i++) {
            ...
            if (client === 'clash') {
                let clashOutbound = buildClashWarpOutbound(
                    i === 1 ? `proxy-${index + 1}` : `X-DNL-X WoW ${index + 1} 🌐`, 
                    warpIPv6, 
                    privateKey, 
                    publicKey, 
                    endpoint,
...
async function buildXrayWorkerLessConfig(remoteDNS, localDNS, lengthMin,  lengthMax,  intervalMin,  intervalMax, fragmentPackets, blockAds, bypassIran, blockPorn, bypassLAN, bypassChina, blockUDP443) {
    ...
    fragConfig.remarks  = 'X-DNL-X  - YouTube(یوتیوب) 📺'
...
async function getXrayFragmentConfigs(env, hostName) {
    ...
    bestPing.remarks = 'X-DNL-X  - بهترین پینگ 🏎️';
    ...
    bestFragment.remarks = 'X-DNL-X  - فرگمنت خودکار 🔥';
...
async function getXrayWarpConfigs (env, client) {
    ...
    xrayWarpBestPing.remarks = client === 'nikang' ? 'X-DNL-X وارپ پرو بهترین پینگ 🏎️' : 'X-DNL-X وارپ بهترین پینگ 🏎️';
    ...
    xrayWarpOutbounds.forEach((outbound, index) => {
        xrayWarpConfigs.push({
            ...xrayWarpConfig,
            remarks: client === 'nikang' ? `X-DNL-X Warp Pro ${index + 1} 🦁☀️` : `X-DNL-X Warp ${index + 1} 🦁☀️`,
    ...
    xrayWoWOutbounds.forEach((outbound, index) => {
        if (outbound.tag === 'chain') {
            ...
            xrayWoWConfig.remarks = client === 'nikang' ? `X-DNL-X WoW Pro ${proxyIndex} 🌐` : `X-DNL-X WoW ${proxyIndex} 🌐`;
    ...
    xrayWoWBestPing.remarks = client === 'nikang' ? 'X-DNL-X وارپ آن وارپ پرو بهترین پینگ 🏎️' : 'X-DNL-X وارپ آن وارپ بهترین پینگ 🏎️';
...
function buildClashChainOutbound(chainProxyParams) {
    ...
    const { hostName, port, uuid, flow, security, type, sni, fp, alpn, pbk, sid, spx, headerType, host, path, authority, serviceName, mode } = chainProxyParams;
    let chainOutbound = {
        "name": "X-DNL-X بهترین پینگ زنجیره🏎️",
        "type": "vless",
        "server": hostName,
        "port": +port,
        "udp": true,
        ...
        "flow": flow,
        "network": type,
        "dialer-proxy": "X-DNL-X بهترین پینگ 🏎️"
    };
...
async function getClashConfig (env, hostName, isWarp) {
    ...
    if (isWarp) {
        config['proxy-groups'][0].proxies = ['X-DNL-X وارپ بهترین پینگ 🏎️', 'X-DNL-X وارپ آن وارپ بهترین پینگ 🏎️'];
        config['proxy-groups'][1].name = 'X-DNL-X وارپ بهترین پینگ 🏎️';
        config['proxy-groups'][1].interval = +bestWarpInterval;
        config['proxy-groups'].splice(2, 0, structuredClone(config['proxy-groups'][1]));
        config['proxy-groups'][2].name = 'X-DNL-X وارپ آن وارپ بهترین پینگ 🏎️';
        ...
        config['proxy-groups'][0].proxies = ['X-DNL-X بهترین پینگ 🏎️'];
        config['proxy-groups'][1].name = 'X-DNL-X بهترین پینگ 🏎️';
...
async function getSingboxConfig (env, hostName, client, isWarp, isFragment) {
    ...
    if (isWarp) {
        ...
        config.outbounds[0].outbounds = client === 'hiddify'
            ? ["X-DNL-X وارپ پرو بهترین پینگ 🏎️", "X-DNL-X وارپ آن وارپ پرو بهترین پینگ 🏎️"]
            : ["X-DNL-X وارپ بهترین پینگ 🏎️", "X-DNL-X وارپ آن وارپ بهترین پینگ 🏎️"];
        config.outbounds.splice(2, 0, structuredClone(config.outbounds[1]));
        config.outbounds[1].tag = client === 'hiddify' 
            ? "X-DNL-X وارپ پرو بهترین پینگ 🏎️"
            : "X-DNL-X وارپ بهترین پینگ 🏎️";
        config.outbounds[2].tag = client === 'hiddify'
            ? "X-DNL-X وارپ آن وارپ پرو بهترین پینگ 🏎️"
            : "X-DNL-X وارپ آن وارپ بهترین پینگ 🏎️";
...
async function getNormalConfigs(env, hostName, client) {
    ...
    if (outProxy) {
        let chainRemark = `#${encodeURIComponent('X-DNL-X Chain proxy 🔗')}`;
    ...
    outbounds: [
        {
            type: "selector",
            tag: "proxy",
            outbounds: ["X-DNL-X بهترین پینگ 🏎️"]
        },
        {
            type: "urltest",
            tag: "X-DNL-X بهترین پینگ 🏎️",
            outbounds: [],
